...
# Configuration
$inputDir = "c:\Users\Zitamine\Victor Dane\Antigravity\Rapoarte\Date_Brute"
$outputFile = "c:\Users\Zitamine\Victor Dane\Antigravity\Proiecte-Varianta-Finala\Raport_Vanzari_2024_2026\sales_data_2024_2025.js"

Write-Host "Fetching latest data from Shopify (Last 7 days)..."
python "c:\Users\Zitamine\Victor Dane\Antigravity\Proiecte-Varianta-Finala\Raport_Vanzari_2024_2026\fetch_shopify_data.py" --days 7
...
foreach ($row in $allRecords) {
    ...
    if ($null -ne $fStatus) {
        ...
        # Allow 'pending' (unpaid) orders per user request
    }

    # Filter test orders (aligned with server.py which checks order.test)
    $notes = $row.Notes
    if ($notes -and $notes -match '\btest\b') { continue }
    ...
    if (-not $processedOrders.ContainsKey($orderId)) {
        ...
        if ($isCanceled) {
            # Canceled orders: skip entirely (aligned with server.py which does `continue`)
            # Still record them for canceled_orders count in aggregation
            $processedOrders[$orderId] = @{
                Name = $orderId; Email = $email; Date = $dateObj
                Month = $monthKey; Year = $year; Quarter = $qtr; Day = $dayKey
                IsCanceled = $true; OrderType = "OTP"; Tags = $tags
                Total = 0; Refunded = 0; Returns = 0; NetSales = 0
                GrossSales = 0; TotalSales = 0; Shipping = 0; Taxes = 0
                DiscountAmount = 0; DiscountCode = $discountCode; IsFirstOrder = $false
            }
            # Do NOT add to customerHistory — cancelled orders don't affect journey/churn
            continue
        }
        ...
        $isFirstOrderByTag = $false
        if ($tagsLower -match "first_order" -or $tagsLower -match "first order") {
            $isFirstOrderByTag = $true
        }
        
        $isSecondOrderByTag = $false
        if ($tagsLower -match "\b2\b" -and $tagsLower -match "recurring") {
            $isSecondOrderByTag = $true
        }
        ...
        $orderObj = @{
            ...
            Tags           = $tags
            IsFirstOrder   = $false # Calculated later by chronology
            IsFirstOrderByTag = $isFirstOrderByTag
            IsSecondOrderByTag = $isSecondOrderByTag
            Shipping       = $shipping
            Taxes          = $taxes
            DiscountAmount = $discountAmount
            DiscountCode   = $discountCode
            HasRecomandate = $false  # Will be set by line item check
            HasPachetOrProtocol = $false  # Will be set by line item check
            HasSingleProduct = $false  # Will be set by line item check
            OrderSource    = "mix"  # Default, will be classified post-processing
        }
    ...
    if ([string]::IsNullOrWhiteSpace($cancelledAtStr)) {
        ...
        # Check line item properties for order source classification
        $itemProps = $row.'Lineitem properties'
        $itemName = $row.'Lineitem name'
        if ($processedOrders.ContainsKey($orderId)) {
            $orderRef = $processedOrders[$orderId]
            if ($itemProps -and $itemProps -match 'Recomandate') {
                $orderRef.HasRecomandate = $true
            }
            if ($itemName -and ($itemName -match 'Pachet|Protocol')) {
                $orderRef.HasPachetOrProtocol = $true
            }
            if ($itemName -and $itemName -notmatch 'Pachet|Protocol') {
                $orderRef.HasSingleProduct = $true
            }
        }
...
# --- Post-Process: Classify Order Source ---
Write-Host "Classifying Order Sources (Chestionar/Predefinite/Mix)..."
foreach ($oid in $processedOrders.Keys) {
    $ord = $processedOrders[$oid]
    if ($ord.HasRecomandate) {
        $ord.OrderSource = "chestionar"
    }
    elseif ($ord.HasPachetOrProtocol -and -not $ord.HasSingleProduct) {
        $ord.OrderSource = "predefinite"
    }
    else {
        $ord.OrderSource = "mix"
    }
...
$srcCounts = @{ chestionar = 0; predefinite = 0; mix = 0 }
foreach ($oid in $processedOrders.Keys) {
    $src = $processedOrders[$oid].OrderSource
    if ($srcCounts.ContainsKey($src)) { $srcCounts[$src]++ }
}
Write-Host "Order Sources: Chestionar=$($srcCounts.chestionar), Predefinite=$($srcCounts.predefinite), Mix=$($srcCounts.mix)"
    ...
    while ($currentSimMonth -lt $simulationEnd) {
        ...
        if ($monthHasOrder.ContainsKey($simKey)) {
            ...
            if (-not $isChurned -and $lastOrderDate -ne $null) {
                ...
                # Rule: SUB3 -> 5 months (Actually 3 months + 2 months grace)
                elseif ($lastOrderType -eq "SUB3" -and $diffMonths -eq 5) {
    ...
    foreach ($oid in $processedOrders.Keys) {
        ...
        if (-not $groups.ContainsKey($key)) {
            $groups[$key] = @{
                ...
                # New Tag-Based Breakdowns
                first_orders_by_type  = @{ OTP = 0; SUB1 = 0; SUB3 = 0; SUB6 = 0 }
                second_orders_total   = 0
                ...
                discount_codes        = @{}
                orders_by_source      = @{ chestionar = @{ orders = 0; revenue = 0.0 }; predefinite = @{ orders = 0; revenue = 0.0 }; mix = @{ orders = 0; revenue = 0.0 } }
            }
        }
        ...
        if (-not $ord.IsCanceled) {
            ...
            # New Tag-based Breakdowns
            if ($ord.IsFirstOrderByTag) {
                $g.first_orders_by_type[$t]++
            }
            if ($ord.IsSecondOrderByTag) {
                $g.second_orders_total++
            }
            ...
            if (-not [string]::IsNullOrWhiteSpace($src)) {
                if (-not $g.orders_by_source.ContainsKey($src)) { 
                    $g.orders_by_source[$src] = @{ orders = 0; revenue = 0.0 } 
                }
                $g.orders_by_source[$src].orders++
                $g.orders_by_source[$src].revenue += $ord.NetSales
            }
        }
        else {
            $g.canceled_orders++
...
$dashboardDest = "c:\Users\Zitamine\Victor Dane\Antigravity\Proiecte-Varianta-Finala\DASHBOARD ZITAMINE\dashboard_data.js"
Copy-Item $dashboardFile -Destination $dashboardDest -Force
Write-Host "Copied dashboard_data.js to $dashboardDest"

Write-Host "Updating conversion data from sessions CSV..."
python "c:\Users\Zitamine\Victor Dane\Antigravity\Proiecte-Varianta-Finala\Raport_Vanzari_2024_2026\process_conversions.py"
...
if ($gitPath) {
    ...
    $repoRoot = "c:\Users\Zitamine\Victor Dane\Antigravity"
