...
def build_google_section(data):
    ...
    # Bug fix: Google Ads Script might export concatenated strings for totalImpressions and Clicks
    real_impr = sum(int(float(c.get('impressions', 0) or 0)) for c in data.get('campaigns', []))
    real_clicks = sum(int(float(c.get('clicks', 0) or 0)) for c in data.get('campaigns', []))
    ...
    return {
        ...
        'currency': 'RON',
        'summary': {
            'totalSpend': round(safe(s.get('totalCost') if 'totalCost' in s else s.get('totalSpend')), 2),
            'totalRevenue': round(safe(s.get('totalConvValue') if 'totalConvValue' in s else s.get('totalRevenue')), 2),
            'roas': round(safe(s.get('totalConvValue') if 'totalConvValue' in s else s.get('totalRevenue')) / max(1, safe(s.get('totalCost') if 'totalCost' in s else s.get('totalSpend'))), 2),
            'totalImpressions': real_impr,
            'totalClicks': real_clicks,
            'avgCTR': round((real_clicks / real_impr * 100) if real_impr > 0 else 0, 2),
            'avgCPC': round((safe(s.get('totalCost') if 'totalCost' in s else s.get('totalSpend')) / real_clicks) if real_clicks > 0 else 0, 2),
            'totalConversions': round(safe(s.get('totalConv') if 'totalConv' in s else s.get('totalConversions')), 0),
            'cpa': round((safe(s.get('totalCost') if 'totalCost' in s else s.get('totalSpend')) / safe(s.get('totalConv') if 'totalConv' in s else s.get('totalConversions'))) if safe(s.get('totalConv') if 'totalConv' in s else s.get('totalConversions')) > 0 else 0, 2),
...
def build_klaviyo_section(data):
    ...
    # Campaigns - top 30 most recent
    raw_campaigns = data.get('campaigns', [])
    raw_campaigns.sort(key=lambda c: str(c.get('sendDate', c.get('sentDate', ''))), reverse=True)
    top_campaigns = raw_campaigns[:30]
...
def build_ecomm_kpis():
    """Build ecommerce-specific KPIs from the monthly marketing report.
    This data comes from the 'Raport lunar - Zitamine.ro.xlsx' Excel file.
    Updated manually when new reports arrive.
    """
    return {
        'reportMonth': 'Martie 2026',
        'metricsConfig': {
             'budget': {'label': 'Budget', 'fmt': 'ron', 'invertColor': True},
             'clicks': {'label': 'Clicks', 'fmt': 'num', 'invertColor': False},
             'avgCPC': {'label': 'Avg. CPC', 'fmt': 'ron2', 'invertColor': True},
             'conversiiOTP': {'label': 'Conversii OTP', 'fmt': 'num', 'invertColor': False},
             'revenueOTP': {'label': 'Revenue OTP', 'fmt': 'ron', 'invertColor': False},
             'cpsOTP': {'label': 'CPS OTP', 'fmt': 'ron2', 'invertColor': True},
             'roasOTP': {'label': 'ROAS OTP', 'fmt': 'num2', 'invertColor': False},
             'cvrOTP': {'label': 'CVR OTP', 'fmt': 'pct', 'invertColor': False},
             'totalBudgetPPC': {'label': 'Total Budget PPC', 'fmt': 'ron', 'invertColor': True},
             'totalSales': {'label': 'Total Sales', 'fmt': 'ron', 'invertColor': False},
             'totalOrders': {'label': 'Total Orders', 'fmt': 'num', 'invertColor': False},
             'totalWebSessions': {'label': 'Total Web Sessions', 'fmt': 'num', 'invertColor': False},
             'roasTotal': {'label': 'ROAS Total', 'fmt': 'num2', 'invertColor': False},
             'cpsTotal': {'label': 'CPS Total', 'fmt': 'ron2', 'invertColor': True},
             'aov': {'label': 'AOV', 'fmt': 'ron', 'invertColor': False},
             'cvr': {'label': 'CVR %', 'fmt': 'pct', 'invertColor': False},
        },
        'google': {
            'current': {
                'budget': 14520.19,
...
def main():
    ...
    # Build ecomm KPIs from monthly report data
    ecomm_kpis = build_ecomm_kpis()
...
"""
    ...
    # Auto-update Javascript Cache Buster in HTML
    html_path = os.path.join(SCRIPT_DIR, 'Marketing_Analytics.html')
    if os.path.exists(html_path):
        with open(html_path, 'r', encoding='utf-8') as html_file:
            html_content = html_file.read()
        # Find script src="ads_data.js..." and replace with current timestamp
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        updated_html = re.sub(r'ads_data\.js(\?v=[0-9]+)?', f'ads_data.js?v={timestamp}', html_content)
